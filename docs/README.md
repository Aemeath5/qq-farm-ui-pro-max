# QQ 农场智能助手 - 文档中心

> 版本：v1.0  
> 更新日期：2026-03-01

---

## 📚 文档导航

### 新手入门

1. **[快速开始](../README.md#快速启动)** - 5 分钟上手
2. **[功能特性](../README.md#功能特性)** - 了解核心功能
3. **[帮助中心](../web/src/views/HelpCenter.vue)** - 内置帮助文档

### 使用指南

4. **[使用手册](./USER_MANUAL.md)** - 完整使用手册与攻略（推荐）
5. **[设置指南](./guides/SETTINGS_GUIDE.md)** - 详细配置说明
6. **[数据库升级](./DATABASE_QUICKSTART.md)** - SQLite 迁移指南
7. **[帮助中心实现](./HELP_CENTER_IMPLEMENTATION.md)** - 帮助系统说明

### 开发文档

8. **[API 参考](./API_REFERENCE.md)** - 完整 API 文档
9. **[配置模板](./CONFIG_TEMPLATES.md)** - 常用配置示例
10. **[性能基准](./PERFORMANCE_BENCHMARK.md)** - 性能测试报告
11. **[Async 调用规范](./ASYNC_调用规范.md)** - Provider 与异步调用规范
12. **[回归测试清单](./REGRESSION_TEST_CHECKLIST.md)** - 功能回归验证清单
13. **[近期优化复盘](./RECENT_OPTIMIZATION_REVIEW_2026-03-08.md)** - 本轮问题、影响与优化建议

### 部署运维

14. **[部署指南](./guides/DEPLOYMENT_GUIDE.md)** - 生产环境部署
15. **[故障排除](./guides/TROUBLESHOOTING.md)** - 常见问题解决
16. **[截图指南](./SCREENSHOT_GUIDE.md)** - 截图最佳实践

### 架构设计

17. **[数据库迁移](./DATABASE_MIGRATION_GUIDE.md)** - 迁移详细过程
18. **[业务架构解析](./业务架构解析/)** - 系统架构说明
19. **[更新日志](../CHANGELOG.DEVELOPMENT.md)** - 版本历史

---

## 🎯 快速查找

### 我想...

**安装和部署**
- → 查看 [部署指南](./DEPLOYMENT_GUIDE.md)
- → 查看 [Docker 部署](./DEPLOYMENT_GUIDE.md#方案三-docker-部署)

**配置和优化**
- → 查看 [使用手册](./USER_MANUAL.md)
- → 查看 [设置指南](./guides/SETTINGS_GUIDE.md)
- → 查看 [配置模板](./CONFIG_TEMPLATES.md)
- → 查看 [性能基准](./PERFORMANCE_BENCHMARK.md)

**使用和开发**
- → 查看 [API 参考](./API_REFERENCE.md)
- → 查看 [故障排除](./TROUBLESHOOTING.md)

**截图和演示**
- → 查看 [截图指南](./SCREENSHOT_GUIDE.md)
- → 查看 [演示视频脚本](./DEMO_VIDEO_SCRIPT.md)

---

## 📋 文档清单

| 文档 | 状态 | 难度 | 时间 |
|------|------|------|------|
| [README](../README.md) | ✅ 完成 | ⭐ | 5 分钟 |
| [使用手册](./USER_MANUAL.md) | ✅ 完成 | ⭐⭐ | 30 分钟 |
| [API 参考](./API_REFERENCE.md) | ✅ 完成 | ⭐⭐⭐ | 30 分钟 |
| [部署指南](./DEPLOYMENT_GUIDE.md) | ✅ 完成 | ⭐⭐ | 20 分钟 |
| [故障排除](./TROUBLESHOOTING.md) | ✅ 完成 | ⭐⭐ | 15 分钟 |
| [配置模板](./CONFIG_TEMPLATES.md) | ✅ 完成 | ⭐⭐ | 20 分钟 |
| [性能基准](./PERFORMANCE_BENCHMARK.md) | ✅ 完成 | ⭐⭐⭐ | 25 分钟 |
| [截图指南](./SCREENSHOT_GUIDE.md) | ✅ 完成 | ⭐ | 10 分钟 |
| [演示视频脚本](./DEMO_VIDEO_SCRIPT.md) | ✅ 完成 | ⭐ | 10 分钟 |

---

## 🔗 外部链接

- **GitHub 仓库**: [your-repo/qq-farm-bot-ui](https://github.com/your-repo/qq-farm-bot-ui)
- **问题反馈**: [GitHub Issues](https://github.com/your-repo/qq-farm-bot-ui/issues)
- **讨论区**: [GitHub Discussions](https://github.com/your-repo/qq-farm-bot-ui/discussions)

---

## 📝 贡献文档

欢迎贡献文档！如果发现错误或需要改进：

1. 提交 Issue 说明问题
2. 提交 PR 修改文档
3. 在讨论区提出建议

---

**最后更新**: 2026-03-01  
**维护人员**: QQ 农场智能助手团队
